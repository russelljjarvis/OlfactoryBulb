...
class LfpElectrode:
    ...
    def setup_neuron_plot_vars(self):
        ...
        h.load_file(os.path.join(dir, 'LFPsimpyValue.hoc'))

        # Initialize it - value is updated in compute()
        self.nrn_value_tracker = h.LFPsimpy()
    ...
    def collect(self):
        ...
        if h.t > h.tstop:
            print('Warning: h.t is more than h.tstop. Please ensure h.tstop is set before '
                  'h.run() or pc.psolve(). Stopping LFP collection.')
            return
